...
# deve ser igual a NUM_LANTERNS em shaders/fragment_shader.fs
# e ao número de instâncias em assets/flying_lantern.py
NUM_LANTERNS = 20
...
class Light:
    ...
    on: bool  # True = luz ligada, False = desligada
    color: tuple  # (R, G, B) cor da luz - valores >1.0 produzem brilho intenso
    positions: list = field(default_factory=lambda: [glm.vec3(0.0)])  # Posição(ões) no espaço
...
def make_default_rig() -> LightingRig:
    ...
    return LightingRig(
        ...
        int_light_b=Light(
            on=True,
            color=(1.5, 0.3, 0.1),
            positions=[glm.vec3(0.0) for _ in range(3)],  # Será preenchido em scene_builder
        ),
    )
